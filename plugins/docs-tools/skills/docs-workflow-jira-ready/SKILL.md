...
All invocations go through `claude -p` so the plugin system resolves script paths via `CLAUDE_PLUGIN_ROOT`. No local checkout of the tools repo is needed — only the plugin installed in Claude Code.

```bash
#!/bin/bash
# docs-workflow-cron.sh — run from crontab
set -a && source ~/.env 2>/dev/null && set +a

# Phase 1: Get ready tickets (through Claude + plugin system)
...
# Phase 2: Run orchestrator for each ticket
...
Then use `fromJson(needs.check.outputs.tickets)` in a matrix strategy to parallelize.
